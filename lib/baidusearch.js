 ...
 */
function BaiduSearch(args, callback) {
  ...
  var hexo = this;
  var config = hexo.config.BaiduSearch;
  ...
  function processPost(post) {
    ...
    object.postId = post.path.replace('/','_'); // use path as key
  ...
  /**
   * Delete the existing index on baidu search
   * no param
   */
  ...
  /**
   * Create a new index on baidu search
   * no param
   */
  ...
  /**
   * Upload generated posts data to Baidu Search
   * no param
   */
  function uploadData() {
    var deferred = Q.defer();
    ...
    request.post(options, function(error, response, body) {
      ...
      if (response.statusCode === 200) {
        log.info("Successfully uploaded index data to Baidu Search.");
      }
      deferred.resolve();
    });
  ...
  function init() {
    ...
    hexo.call('clean', function(err) {
      ...
      hexo.call('generate', {}).then(function(err) {
        ...
        // TODO: upload posts to Baidu Search
        deleteIndex()
          .then(createIndex)
          .then(uploadData);
      });
